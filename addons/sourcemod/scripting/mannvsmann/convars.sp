...
void ConVars_Init()
{
	...
	sm_mvm_currency_rewards_player_count_base = CreateConVar("sm_mvm_currency_rewards_player_count_base", "32", "The base amount of players to use for currency scaling.", _, true, 0.0, true, float(MAXPLAYERS - 1));
	...
	sm_mvm_currency_rewards_player_count_bonus_max = CreateConVar("sm_mvm_currency_rewards_player_count_bonus_max", "2", "Maximum multiplier to currency when player count drops below base amount.", _, true, 1.0);
	...
	sm_mvm_catchup_base_threshold = CreateConVar("sm_mvm_catchup_base_threshold", "100", "Base credit difference before catchup begins", _, true, 0.0);
	sm_mvm_catchup_threshold_scale = CreateConVar("sm_mvm_catchup_threshold_scale", "0.1", "How much the threshold scales with total credits, in percent.", _, true, 0.0, true, 1.0);
	sm_mvm_catchup_multiplier_strength = CreateConVar("sm_mvm_catchup_multiplier_strength", "0.15", "How strong the catchup effect is per threshold, in percent.", _, true, 0.0, true, 1.0);
	sm_mvm_currency_rewards_player_modifier_arena = CreateConVar("sm_mvm_currency_rewards_player_modifier_arena", "1.66", "Multiplier to dropped currency in arena mode.");
